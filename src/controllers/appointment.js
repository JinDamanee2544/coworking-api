...
exports.addAppointment=async(req,res,next)=>{
    try{
        ...
        for(let i=0;i<allAppointments.length;i++){
            ...
            if(allAppointments[i].apptDate.toDateString() == req.body.apptDate.toDateString()&& id === req.body.space.toString() &&
            isOverlap(req.body.startTime,req.body.endTime,allAppointments[i].startTime,allAppointments[i].endTime)
            ){
...
exports.updateAppointment=async(req,res,next)=>{
    try{
        ...
        for(let i=0;i<allAppointments.length;i++){
            const id =allAppointments[i].space._id.toString();
            if(allAppointments[i]._id!==req.params.id &&
                allAppointments[i].apptDate.toDateString() == req.body.apptDate.toDateString()&& id === req.body.space.toString() &&
            isOverlap(req.body.startTime,req.body.endTime,allAppointments[i].startTime,allAppointments[i].endTime)
            ){
                return res.status(400).json({success:false,message:'Cannot reserve with time overlap to other appointments'});
            }
